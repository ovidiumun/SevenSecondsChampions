...
class GameCenter {
    ...
    func authenticateLocalPlayer(presentingVC: UIViewController) -> String {
        ...
        localPlayer.authenticateHandler = { [weak self] (gameCenterViewController, error) -> Void in
            ...
            if error != nil {
                ...
                errorGlobal = "Show login if player is not logged in"
                ...
                errorGlobal = "Player is already authenticated & logged in, load game center"
                self?.isGameCenterEnabled = true
            } else {
                // 3. Game center is not enabled on the users device
                self?.isGameCenterEnabled = false
                errorGlobal = "Game center is not enabled on the users device"
